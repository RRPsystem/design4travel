#!/usr/bin/env bash
# C05 — save_document_internal vs remove_member op dezelfde user.
# Verwacht: één slaagt, één faalt cleanly.
#   Als remove eerst commit → save raise membership_not_active
#   Als save eerst commit  → remove verwijdert daarna netjes

set -euo pipefail
DBURL="${DATABASE_URL:-postgresql://postgres:postgres@localhost:54322/postgres}"

psql "$DBURL" -v ON_ERROR_STOP=1 -q -f "$(dirname "$0")/_seed.sql" >/dev/null

# Extra: maak een project met een doc voor user 'editor' (id 3)
OWNER='11111111-0000-0000-0000-000000000001'
EDITOR='33333333-0000-0000-0000-000000000003'
ORG=$(psql "$DBURL" -tAc "select id from public.organizations where slug='race-team'")

# Project + document seeden
psql "$DBURL" -v ON_ERROR_STOP=1 -q <<SQL >/dev/null
select set_config('request.jwt.claims',
  '{"sub":"$OWNER","role":"authenticated"}', false);
select set_config('role','authenticated', false);
select public.create_project('$ORG', 'Race project', 'website', null);
SET ROLE postgres;
insert into public.project_documents (project_id, doc, schema_version, updated_by)
select p.id,
       jsonb_build_object(
         'version','0.1.0',
         'project', jsonb_build_object('documentType','website','title','x'),
         'pages',   jsonb_build_array(jsonb_build_object('id','p','root',jsonb_build_object('id','r','type','text','props','{}'::jsonb)))
       ),
       '0.1.0',
       '$OWNER'
  from public.projects p where p.name='Race project';
RESET ROLE;
SQL

PROJECT=$(psql "$DBURL" -tAc "select id from public.projects where name='Race project'")
EDITOR_MEM=$(psql "$DBURL" -tAc "select id from public.organization_members where user_id='$EDITOR' and organization_id='$ORG' and deleted_at is null")

tmp1=$(mktemp); tmp2=$(mktemp)
(
  # save als editor (via service_role, EF-stijl)
  psql "$DBURL" -v ON_ERROR_STOP=1 -c "
    begin;
    SET ROLE service_role;
    select public.save_document_internal(
      '$EDITOR'::uuid,
      '$PROJECT'::uuid,
      '{\"version\":\"0.1.0\",\"project\":{\"documentType\":\"website\",\"title\":\"new\"},\"pages\":[{\"id\":\"p\",\"root\":{\"id\":\"r\",\"type\":\"text\",\"props\":{}}}]}'::jsonb,
      '0.1.0', 1);
    RESET ROLE;
    commit;
  " >"$tmp1" 2>&1
) &
p1=$!
(
  # remove editor (door owner)
  psql "$DBURL" -v ON_ERROR_STOP=1 -c "
    begin;
    select set_config('request.jwt.claims',
      '{\"sub\":\"$OWNER\",\"role\":\"authenticated\"}', false);
    select set_config('role','authenticated', false);
    select public.remove_member('$EDITOR_MEM');
    commit;
  " >"$tmp2" 2>&1
) &
p2=$!
set +e; wait $p1; r1=$?; wait $p2; r2=$?; set -e

editor_active=$(psql "$DBURL" -tAc "
  select case when deleted_at is null then 'yes' else 'no' end
  from public.organization_members where id='$EDITOR_MEM'")
doc_lock=$(psql "$DBURL" -tAc "select lock_version from public.project_documents where project_id='$PROJECT'")

echo "C05 results: save_tx=$r1 remove_tx=$r2 editor_active=$editor_active doc_lock=$doc_lock"

# Valide uitkomsten:
#  (a) save wint: r1=0, r2=0 (remove wacht op FOR UPDATE lock, commit erna),
#      editor_active=no na commit, doc_lock=2
#  (b) remove wint: r2=0, r1!=0 met membership_not_active,
#      editor_active=no, doc_lock=1
if [[ $r1 -eq 0 && $r2 -eq 0 && "$editor_active" == "no" && "$doc_lock" == "2" ]]; then
  echo "PASS: save-first-then-remove"; rm -f "$tmp1" "$tmp2"; exit 0
fi
if [[ $r1 -ne 0 && $r2 -eq 0 && "$editor_active" == "no" && "$doc_lock" == "1" ]] &&
   grep -qE 'membership_not_active|serialization_failure' "$tmp1"; then
  echo "PASS: remove-first-blocks-save"; rm -f "$tmp1" "$tmp2"; exit 0
fi

echo "FAIL"; cat "$tmp1"; cat "$tmp2"; rm -f "$tmp1" "$tmp2"; exit 1
