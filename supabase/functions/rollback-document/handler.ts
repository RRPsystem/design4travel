...
export function makeHandler(deps: HandlerDeps) {
  return async function handle(req: Request): Promise<Response> {
    ...
    // The try/catch fail-closes any unexpected body-stream rejection (client
    // disconnect, malformed transfer-encoding, controller.error, ...) to
    // 500 internal_error with our contract body + CORS. No dependency,
    // auth, or database work follows.
    ...
    const parsed = RollbackRequestSchema.safeParse(raw);
    ...
    try {
      rpcResult = (await admin.rpc("rollback_to_version", {
        p_actor_user_id: actorUserId,
        p_project_document_id: input.project_document_id,
        p_target_version_number: input.target_version_number,
    ...
    const row = RollbackResponseSchema.safeParse(rpcResult.data[0]);
